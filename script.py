...
jeopardy_data = jeopardy_data.rename(
    columns={" Air Date": "Air Date", " Round": "Round", " Category": "Category", " Value": "Value",
...
# Define a function to convert to float
# jeopardy_data["Float Value"] = jeopardy_data["Value"].apply(lambda x: float(x[1:].replace(',','')) if x != "None" else 0)    # old code
def convert_to_float(value):
    if isinstance(value, str) and value != "None":
        return float(value[1:].replace(',', ''))  # Convert to float after removing '$' and ','
    return 0  # Return 0 for None or non-string values

# Apply the function to create the "Float Value" column
jeopardy_data["Float Value"] = jeopardy_data["Value"].apply(convert_to_float)
